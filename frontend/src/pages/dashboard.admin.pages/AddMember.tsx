          ...
          <div className="max-w-4xl mx-auto">
            ...
            <section className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-3xl font-bold text-blue-700 dark:text-gray-100">Add New Member</h1>
                  <p className="mt-1 text-gray-600 dark:text-gray-300">Register a new member to your Njangi circle.</p>
                </div>
              </div>
              <span className="mt-4 md:mt-0 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-200 text-xs font-semibold">{new Date().toLocaleDateString()}</span>
            </section>
            ...
            <section className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-10">
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {/* Phone */}
                <div>
                  <label className="block text-gray-700 dark:text-gray-200 font-medium mb-1">Phone</label>
                  <PhoneInput
                    country="cm"
                    value={form.phone}
                    onChange={phone => { setForm({ ...form, phone }); setErrors({ ...errors, phone: '' }); setSuccess(null); }}
                    containerClass="w-full"
                    inputClass={`w-full px-4 py-2 rounded border focus:outline-none focus:ring-2 focus:ring-blue-400 dark:bg-gray-900 dark:text-white ${errors.phone ? 'border-red-500' : 'border-gray-300 dark:border-gray-700'}`}
                ...
                <div className="flex justify-end mt-6">
                  ...
                  {success && <span className="ml-4 text-green-600 dark:text-green-400">{success}</span>}
                </div>
              </form>
            </section>
            ...
            <section className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              <h2 className="text-lg font-bold text-green-700 dark:text-gray-100 mb-4">Current Members</h2>
              <div className="overflow-x-auto">
                <MemberTable members={[]} onDelete={id => console.log(`Delete ${id}`)} />
              </div>
            </section>
          </div>
        </main>
