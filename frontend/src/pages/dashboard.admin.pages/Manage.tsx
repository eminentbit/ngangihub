        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            <button
              ...
              className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 dark:bg-indigo-600 dark:hover:bg-indigo-700"
            >
              + Add New Member
            </button>
          </div>
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={member.id} className="hover:bg-blue-50 dark:hover:bg-gray-700 transition">
                    <td className="py-4 px-6">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 dark:from-indigo-700 dark:to-purple-700 flex items-center justify-center text-white font-bold shadow">
                          {member.initials}
                        </div>
                        <span className="ml-3 text-blue-900 dark:text-gray-100 font-medium">{member.name}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6 text-blue-700 dark:text-gray-300">{member.role}</td>
                    <td className="py-4 px-6">
                      <span className={`text-xs px-3 py-1 rounded-full font-semibold ${member.status==='Active'? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300':'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300'}`}>
                        {member.status}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-right flex justify-end gap-2">
                      <button onClick={()=>openEditModal(member)} aria-label="Edit member" className="p-2 hover:bg-blue-100 dark:hover:bg-gray-700 rounded">
                        <FaEdit />
                      </button>
                      <button onClick={()=>handleDelete(member.id)} aria-label="Remove member" className="p-2 hover:bg-red-100 dark:hover:bg-gray-700 rounded">
                        <FaTrash />
                      </button>
                    </td>
                  </tr>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
                ...
                <form onSubmit={handleSave} className="space-y-4">
                  {['Full Name', 'Role'].map(label => {
                    const key = label.replace(' ', '').toLowerCase() as keyof Member;
                    return <div key={key}>
                      <label className="block text-sm font-medium text-blue-700 dark:text-gray-300 mb-1" htmlFor={key}>
                        {label}
                      </label>
                      <input
                        id={key}
                        name={key}
                        type="text"
                        defaultValue={editingMember ? editingMember[key] : ''}
                        required
                        className="w-full border border-blue-200 dark:border-gray-700 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-gray-500"
                      />
                    </div>;
                  })}
                  <div>
                    <label className="block text-sm font-medium text-blue-700 dark:text-gray-300 mb-1" htmlFor="status">
                      Status
                    </label>
                    <select
                      id="status"
                  ...
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={closeModal} className="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600">
                      Cancel
                    </button>
                    <button type="submit" className="px-4 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-600 dark:bg-indigo-600 dark:hover:bg-indigo-700">
