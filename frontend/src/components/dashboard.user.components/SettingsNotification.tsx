...
const notificationSettings = [
  { key: 'email', label: 'Email Notifications', desc: 'Get email updates for important activities' },
  { key: 'push', label: 'Push Notifications', desc: 'Receive notifications directly on your device' },
  { key: 'reminders', label: 'Payment Reminders', desc: 'Be reminded of upcoming payments' },
  { key: 'activity', label: 'Group Activity', desc: 'Stay in the loop with group actions' },
];
...
export default function SettingsNotifications() {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white dark:bg-gray-800 shadow-xl rounded-2xl">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">Notification Settings</h2>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">Customize how you receive updates and alerts.</p>
      ...
      <div className="space-y-4">
        {notificationSettings.map(({ key, label, desc }) => (
          <div key={key} className="flex items-center justify-between p-4 border dark:border-gray-700 rounded-lg">
            ...
            <button
              type="button"
              onClick={() => handleToggle(key as keyof SettingsNotificationsProps)}
