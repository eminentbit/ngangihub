...
export default function InviteTokenStatusUi({ inviteStatus, itemType = "invitation" }: InviteTokenStatusUiProps) {
  const navigate = useNavigate()

  const getStatusConfig = () => {
    const itemName = itemType.toLowerCase()
    const itemNameCapitalized = itemType.charAt(0).toUpperCase() + itemType.slice(1)
    ...
    switch (inviteStatus) {
      ...
      case "valid":
        return {
          icon: <AlertCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />,
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center bg-gradient-to-br ${config.bgGradient} p-4`}>
      <div className="bg-white/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-white/20 max-w-md w-full text-center">
        ...
        <div className="mt-6 text-sm text-gray-500">Need help? Contact your administrator</div>
      </div>
    </div>
  )
}
