...
const ReportDetails: React.FC<ReportDetailsProps> = ({ reportId, isDarkMode, onBack }) => {
  const reportsDetails = {
    1: {
      ...
      content: 'Revenue: $10M, Expenses: $7M, Net Profit: $3M. Key highlights include a 15% revenue growth and a 10% reduction in operational costs.',
    ...
    2: {
      ...
      content: 'Operational uptime: 98%, Employee satisfaction: 85%. Notable improvements in process efficiency and staff training programs.',
    ...
    3: {
      ...
      content: 'Goals include market expansion, sustainability focus, and technology upgrades. Budget allocated: $5M.',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
        ...
        <h3 style={{ margin: '0', fontSize: '18px', color: isDarkMode ? 'white' : 'black' }}>{report.title}</h3>
      </div>
      ...
      {report.approvedDate && (
        <p style={{ color: isDarkMode ? '#d1d5db' : '#6b7280', fontSize: '14px' }}>
          <strong>Approved:</strong> {new Date(report.approvedDate).toLocaleString()}
        </p>
      )}
      <p style={{ color: isDarkMode ? '#d1d5db' : '#6b7280', fontSize: '14px' }}>
        <strong>Summary:</strong> {report.summary}
      </p>
      <h4 style={{ fontSize: '16px', color: isDarkMode ? 'white' : 'black', margin: '16px 0 8px' }}>Full Report</h4>
      <p style={{ color: isDarkMode ? '#d1d5db' : '#6b7280', fontSize: '14px' }}>
        {report.content}
      </p>
      <h4 style={{ fontSize: '16px', color: isDarkMode ? 'white' : 'black', margin: '16px 0 8px' }}>Key Metrics</h4>
