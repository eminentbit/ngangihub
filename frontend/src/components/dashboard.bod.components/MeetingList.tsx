...
const MeetingList: React.FC<MeetingListProps> = ({ isDarkMode, filter }) => {
  ...
  const filteredMeetings = filter === 'All' ? meetings : meetings.filter(meeting => meeting.status === filter);
  ...
  return (
    <div style={{
      ...
      <h3 style={{ margin: '0 0 16px 0', fontSize: '18px', color: isDarkMode ? 'white' : 'black' }}>Scheduled Meetings</h3>
      {filteredMeetings.length === 0 ? (
        <p style={{ color: isDarkMode ? '#d1d5db' : '#6b7280' }}>No meetings found.</p>
      ) : (
        filteredMeetings.map(meeting => (
          <MeetingCard key={meeting.id} meeting={meeting} isDarkMode={isDarkMode} isMobile={isMobile} />
        ))
      )}
    </div>
  );
